...
\node [Text, left of=ULF, text width=12em, node distance=17em] (ULF-lower-bound) {$\leadsto$ Untere Schranke: $\OPT(\CLR) \geq \OPT(\ULF)$};
\node [Alg, below of=ULF, text width=14em] (ULF-solving) {löse approximativ (mit Greedy)};
\node [Image, left of=ULF-solving, node distance=16.5em] (ULF-img) {\includegraphics[width=5em]{bilder/ULF.pdf}};
...
\node [Text, right of=MST, text width=12em, node distance=17em] (MST-lower-bound) {$\leadsto$ Untere Schranke: $\OPT(\CLR) \geq \OPT(\MST)$};
\node [Alg, below of=MST, text width=14em] (MST-solving) {löse exakt $\leadsto$ Spannbaum $B$};
...
\node [Text, left of=m-facilities, node distance=25em, text width=15em] () 
	{Kosten beschränkt durch die entsprechenden Eröffnungskosten in $\ULF$ und $\MST$};
...
\node [Text, left of=m-big-clients, node distance=25em, text width=15em] () 
	{Kosten beschränkt durch $2\times$ den entsprechenden Verbindungskosten in $\ULF$};
...
\node [Alg, below of=m-small-clients, node distance=7em] (m-relieve) {
		\begin{minipage}{30em}\begin{algorithmic}\algtext*{For}\algtext*{EndFor}
			...
			\State Partitioniere die Menge der Unterbäume von $B_v$ in Mengen mit Bedarf zw. $u/2$ und $u$
			\State Wandle diese zu Touren um und verbinde sie mit nächster offener Fabrik
			\EndWhile
			\EndFor
			\end{algorithmic}\end{minipage}
	};
...
\node [Text, left of=m-relieve, node distance=25em, text width=15em] () 
	{Kosten beschränkt durch $2\times$ die Kantenkosten im $\MST$ und $2\times$ den Verbindungskosten aller Knoten der Tour in $\ULF$};

% Rest
\node [Alg, below of=m-relieve, node distance=7em] (m-remaining) {
		\begin{minipage}{30em}\begin{algorithmic}\algtext*{For}
		\For
		\State Übrige Kunden in $B_f$ (mit Gesamtbedarf $\leq u$) zu Tour verbinden und mit nächster offener Fabrik verbinden
		\EndFor
		\end{algorithmic}\end{minipage}
	};
